...
__device__ __forceinline__ void load_matrix_x2(unsigned int (&fragment)[2], unsigned int address) {
    asm volatile(
...
__device__ __forceinline__ void load_matrix_x2_transpose(unsigned int (&fragment)[2], unsigned int address) {
    asm volatile(
...
__device__ __forceinline__ void load_right_transposed_fragment(unsigned int (&fragment)[2], const __nv_bfloat16* shared, int tile_inner, int tile_column,
                                                               int lane) {
    const int row = tile_column * MMA_N + lane % MMA_N;
    const int column = tile_inner * MMA_K + (lane % MMA_M) / 8 * 8;
    load_matrix_x2(fragment, shared_address(shared + swizzle(row * D + column)));
}

__device__ __forceinline__ void load_right_fragment(unsigned int (&fragment)[2], const __nv_bfloat16* shared, int tile_inner, int tile_column, int lane) {
    const int row = tile_inner * MMA_K + lane % MMA_M;
    const int column = tile_column * MMA_N;
    load_matrix_x2_transpose(fragment, shared_address(shared + swizzle(row * D + column)));
}
...
#pragma unroll
        for (int tile_column = 0; tile_column < N_TILES; ++tile_column) {
            ...
            load_right_transposed_fragment(right_fragment, right, tile_inner, tile_column, lane);
...
#pragma unroll
        for (int tile_column = 0; tile_column < N_TILES; ++tile_column) {
            unsigned int right_fragment[2];
            load_right_fragment(right_fragment, right, tile_inner, tile_column, lane);
            mma(accumulators[tile_column], left[tile_inner], right_fragment);
        }
    }
}
...
#pragma unroll
        for (int tile = 0; tile < SCORE_N_TILES; ++tile) {
            ...
            if (first_key == first_query) {
                scores[tile][0] = local_top_row >= key_column ? scores[tile][0] * scale : -FLT_MAX;
                scores[tile][1] = local_top_row >= key_column + 1 ? scores[tile][1] * scale : -FLT_MAX;
                scores[tile][2] = local_bottom_row >= key_column ? scores[tile][2] * scale : -FLT_MAX;
                scores[tile][3] = local_bottom_row >= key_column + 1 ? scores[tile][3] * scale : -FLT_MAX;
            } else {
                scores[tile][0] *= scale;
                scores[tile][1] *= scale;
                scores[tile][2] *= scale;
        ...
        const float next_max[2] = {fmaxf(row_max[0], current_max[0]), fmaxf(row_max[1], current_max[1])};
        const float previous_scale[2] = {expf(row_max[0] - next_max[0]), expf(row_max[1] - next_max[1])};
#pragma unroll
...
#pragma unroll
        for (int tile = 0; tile < SCORE_N_TILES; ++tile) {
            probability_top[tile] = make_float2(expf(scores[tile][0] - next_max[0]), expf(scores[tile][1] - next_max[0]));
            probability_bottom[tile] = make_float2(expf(scores[tile][2] - next_max[1]), expf(scores[tile][3] - next_max[1]));
            local_sum[0] += probability_top[tile].x + probability_top[tile].y;
            local_sum[1] += probability_bottom[tile].x + probability_bottom[tile].y;
        }
        ...
        pack_row_matrix(probability_fragments, probability_top, probability_bottom);
...
#pragma unroll
    for (int tile = 0; tile < OUTPUT_N_TILES; ++tile) {
        ...
        const float2 top = make_float2(output_accumulators[tile][0] / row_sum[0], output_accumulators[tile][1] / row_sum[0]);
        const float2 bottom = make_float2(output_accumulators[tile][2] / row_sum[1], output_accumulators[tile][3] / row_sum[1]);
        store_pair(output + tensor_index(batch, top_query, head, column, sequence_length, heads), top);
        store_pair(output + tensor_index(batch, bottom_query, head, column, sequence_length, heads), bottom);
    }
    if (lane % 4 == 0) {
        logsumexp[batch_head * sequence_length + top_query] = row_max[0] + logf(row_sum[0]);
        logsumexp[batch_head * sequence_length + bottom_query] = row_max[1] + logf(row_sum[1]);
    }
}
...
void launch_forward(__nv_bfloat16* output, float* logsumexp, const __nv_bfloat16* query, const __nv_bfloat16* key, const __nv_bfloat16* value, int batch_size,
                    int sequence_length, int heads, float scale, cudaStream_t stream) {
    const dim3 grid(sequence_length / BM, batch_size * heads);
    flash_attention_forward_tensor_core_kernel<<<grid, THREADS, 0, stream>>>(output, logsumexp, query, key, value, sequence_length, heads, scale);
...
void flash_attention_forward_sm89_cuda(__nv_bfloat16* output, float* logsumexp, const __nv_bfloat16* query, const __nv_bfloat16* key,
                                       const __nv_bfloat16* value, int batch_size, int sequence_length, int heads, int head_size, float scale,
                                       cudaStream_t stream) {
    ...
    tensor_core::launch_forward(output, logsumexp, query, key, value, batch_size, sequence_length, heads, scale, stream);
}
...
void flash_attention_backward_sm89_cuda(__nv_bfloat16* query_gradient, __nv_bfloat16* key_gradient, __nv_bfloat16* value_gradient,
    ...
    tensor_core::launch_backward(query_gradient, key_gradient, value_gradient, output_gradient, output, logsumexp, query, key, value, batch_size,
                                 sequence_length, heads, scale, stream);
}
