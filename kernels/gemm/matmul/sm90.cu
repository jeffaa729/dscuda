...
template <int ScaleD>
__device__ __forceinline__ void wgmma_m64n64k16(float (&accumulator)[4][8], bf16* left, bf16* right) {
...
__global__ __launch_bounds__(NUM_THREADS) void gemm_bf16_kernel(bf16* __restrict__ output, const __grid_constant__ CUtensorMap left_map,
                                                                const __grid_constant__ CUtensorMap right_map, int M, int N, int K) {
#if defined(__CUDA_ARCH__) && __CUDA_ARCH__ >= 900
    ...
    for (int tile_k = 0; tile_k < K / BK; ++tile_k) {
        ...
        if (threadIdx.x == 0) {
            cde::cp_async_bulk_tensor_2d_global_to_shared(shared_left, &left_map, tile_k * BK, tile_m * BM, left_barrier);
            left_token = cuda::device::barrier_arrive_tx(left_barrier, 1, sizeof(shared_left));
            cde::cp_async_bulk_tensor_2d_global_to_shared(shared_right, &right_map, tile_n * BN, tile_k * BK, right_barrier);
            right_token = cuda::device::barrier_arrive_tx(right_barrier, 1, sizeof(shared_right));
        } else {
        ...
        wgmma_m64n64k16<1>(accumulator, shared_left + WGMMA_K, shared_right + WGMMA_K * BN);
        wgmma_m64n64k16<1>(accumulator, shared_left + 2 * WGMMA_K, shared_right + 2 * WGMMA_K * BN);
        wgmma_m64n64k16<1>(accumulator, shared_left + 3 * WGMMA_K, shared_right + 3 * WGMMA_K * BN);
        warpgroup_commit();
        warpgroup_wait();
    }
...
#define STORE(Row, Column, Value) tile_output[(Row) * N + (Column)] = __float2bfloat16(Value)
...
void gemm_bf16_sm90_cuda(bf16* output, const bf16* left, const bf16* right, int M, int N, int K, cudaStream_t stream) {
    ...
    gemm_bf16_kernel<<<grid, NUM_THREADS, 0, stream>>>(output, left_map, right_map, M, N, K);
